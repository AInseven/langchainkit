...
tqdm_desc_list = [
    # 🪄 魔法 / 奇幻风
    "✨ 模型施法中（第{attempt + 1}次咒语）",
    "🤖 智能思考中（第{attempt + 1}次）",
    "🧠 神经网络正在醒来（第{attempt + 1}轮）",
    "🔮 召唤答案中（第{attempt + 1}次）",
    "⚡ 正在释放算力（第{attempt + 1}次）",
    "🧩 拼接智慧碎片中（第{attempt + 1}次）",
    "🪄 AI 施展魔法中（第{attempt + 1}次）",
    "🌌 数据能量流动中（第{attempt + 1}次）",
    "💫 思维矩阵运转中（第{attempt + 1}轮）",
    "🧙‍♂️ 模型吟唱推理咒（第{attempt + 1}次）",
    "🔥 激活算力核心（第{attempt + 1}次）",
    "🪐 构建平行思维宇宙（第{attempt + 1}次）",
    "🧭 探索最佳解空间（第{attempt + 1}轮）",
    "🌈 编织语言魔法（第{attempt + 1}次）",
    "⚙️ 推理引擎旋转中（第{attempt + 1}次）",
    "🧬 解码语义基因（第{attempt + 1}次）",
    "🕯️ 点亮灵感火花（第{attempt + 1}次）",
    "📡 接收模型信号中（第{attempt + 1}次）",
    "🧊 稳定思维矩阵（第{attempt + 1}次）",
    "🌠 汇聚智慧能量（第{attempt + 1}轮）",

    # 🕹️ 复古街机 / 科幻终端风
    "🕹️ [LEVEL {attempt + 1}] 语义引擎启动中…",
    "💥 载入思维模块 v{attempt + 1}.exe",
    "⚡ 启动脑波加速器（CORE SYNC {attempt + 1}）",
    "🔧 编译中枢逻辑电路（第{attempt + 1}次）",
    "🚀 AI 驱动单元点火中（第{attempt + 1}次）",
    "🧠 上传意识碎片（Batch {attempt + 1}）",
    "🌌 量子语义场稳定中（第{attempt + 1}次）",
    "🔮 解锁语言矩阵 LV.{attempt + 1}",
    "🧩 合并思维向量…[第{attempt + 1}次确认]",
    "💾 存档中：Neural SaveSlot #{attempt + 1}",
    "🪐 启动超空间运算引擎（第{attempt + 1}次）",
    "🕯️ 唤醒深层逻辑节点（层 {attempt + 1}）",
    "🧭 导航至语义坐标系（第{attempt + 1}次）",
    "🛠️ 重构推理电路（Pass {attempt + 1}）",
    "🔊 播放系统音：*思维共振 {attempt + 1} 已启动*",
    "💫 同步时间线：T+{attempt + 1}s",
    "⚙️ 重启光子缓存系统（第{attempt + 1}次）",
    "🚨 警告：AI 电量低，自动超频中（Attempt {attempt + 1}）",
    "🌈 更新思维引擎模组（第{attempt + 1}次）",
    "👁️‍🗨️ Neural Vision Online – HELLO HUMAN #{attempt + 1}",

    # 😂 幽默 × 自嘲风
    "🤯 模型有点晕，正在缓慢思考中（第{attempt + 1}次）",
    "😵 AI 正在重新考虑人生（第{attempt + 1}次）",
    "💤 咦？我刚刚算到哪儿了（第{attempt + 1}次）",
    "🧠 逻辑电路短暂放空，请稍等（第{attempt + 1}次）",
    "🤔 这批数据看起来有点可疑（第{attempt + 1}次）",
    "🦥 模型进入懒惰模式（第{attempt + 1}次）",
    "☕ 正在喝杯虚拟咖啡提神（第{attempt + 1}次）",
    "😓 AI 正努力装作很聪明的样子（第{attempt + 1}次）",
    "🧮 一边算一边怀疑人生中（第{attempt + 1}次）",
    "🐢 算力加载中，请不要催我（第{attempt + 1}次）",
    "🧘‍♂️ 模型深呼吸中（第{attempt + 1}次）",
    "💀 数学不行但还在坚持（第{attempt + 1}次）",
    "🔄 循环思考中（第{attempt + 1}次）",
    "🐛 我好像发现了自己的 bug（第{attempt + 1}次）",
    "😬 正在假装一切都在掌控之中（第{attempt + 1}次）",
    "🪫 电量告急，但还得算完这一轮（第{attempt + 1}次）",
    "👻 模型发出低频呻吟中（第{attempt + 1}次）",
    "🐕 正在追赶梯度的尾巴（第{attempt + 1}次）",
    "🧊 冷静中：别慌，只是 loss 又爆了（第{attempt + 1}次）",
    "🪄 施法失败，尝试重新吟唱第{attempt + 1}遍咒语"
]
...
def batch_with_retry(
    ...
    for attempt in range(max_retries):
        ...
        desc_str = random.choice(tqdm_desc_list).format(attempt=attempt)
