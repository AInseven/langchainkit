...
def _prepare_parsing_chain(
    ...
    """
    Internal helper to prepare chain and configs for prompt_parsing.

    Returns: (chain, invoke_configs, model_name)
    """
    ...
    return chain, invoke_configs, model_name
...
def prompt_parsing(model: Type[M],
    ...
    # 1.0
    """
    chain, invoke_configs, model_name = _prepare_parsing_chain(
        model, query, llm, use_langfuse, langfuse_user_id,
...
async def aprompt_parsing(model: Type[M],
    ...
    Example:
    from langchainkit import aprompt_parsing,LocalLLM
    from pydantic import BaseModel
    import asyncio
    ...
    # 如果query是单个请求str，则直接调用
    if isinstance(query, str):
        return await chain.ainvoke({"query": query}, config=invoke_configs)

    # 如果query是多个请求list[str]，则使用abatch_with_retry批量调用
    inputs = [{"query": q} for q in query]

    results = await abatch_with_retry(
        llm=chain,
        prompts=inputs,
        input_config=invoke_configs,
        max_retries=10,
